...
export default async function listUserFolders(node, sorceDir, destDir, newName) {
  return getStdout(node.hostname, node.winrmCredential.user, node.winrmCredential.encryptedPassword, build(node.importDirectoryRoot, sorceDir, destDir, newName));
}
