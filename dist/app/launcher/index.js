...
async function scanMissingJobs() {
    ...
    toBeMissing.forEach((job) => {
        jobStatusReciever.missing(job, 'The status is missed because Abaqus Manager was maybe halted while the job was running or starting.');
    });
}
export default async () => {
    ...
    // 10秒間隔で実行
    const task = schedule.schedule('*/10 * * * * *', asyncCallback(async () => {
