...
export async function listUserFolders(node, configFileName) {
    return (await getJSON(node.hostname, node.winrmCredential.user, node.winrmCredential.encryptedPassword, build('listUserFolders.ps1', node.importDirectoryRoot, configFileName))).directories;
}
