...
export default async () => {
  ...
  launcher.on('queue', (job: Job, count: number) => {
    logger.verbose(`Queue ${job.owner}'s job: ${job.name} on ${count}th`);
  });
  launcher.on('start', (job: Job, _count: number) => {
