...
import type { Job, Node } from 'model/resources/types.js'
...
export type LaunchEvent = typeof LaunchEvent[keyof typeof LaunchEvent]
...
export default class JobLauncher extends EventEmitter {
  ...
  constructor() {
    ...
    this.queue.error((err, params) => {
      this.emit(LaunchEvent.ERROR, params.job.toObject(), err);
    });
  }
  ...
  launch(job: Job) {
    ...
    if (count > 1) {
      this.emit(LaunchEvent.QUEUE, job.toObject(), count);
    }
  }
}

const datePostfixFormat = 'yyyymmddHHMMssl';
async function launchJob(job: Job, emitter: JobLauncher) {
  emitter.emit(LaunchEvent.START, job.toObject());

  const datePostfix = dateformat(Date.now(), datePostfixFormat);
  const workingDirName = `${job.owner}_${job.name}_${datePostfix}`;
  const node = (await NodeModel.findOne({ hostname: job.node }) as Node).toObject();
  ...
  psRemote
    ...
    .on('error', error => {
      emitter.emit(LaunchEvent.ERROR, job.toObject(), error);
    })
    .on('finish', async (code, lastStdOut) => {
      ...
      emitter.emit(LaunchEvent.FINISH, job.toObject(), code, msg, resultDir);
    })
    .invoke();

  emitter.emit(LaunchEvent.LAUNCH, job.toObject(), path.join(node.executeDirectoryRoot, workingDirName));
};
