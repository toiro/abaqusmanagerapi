...
router.get('/', async (ctx, _next) => {
  await tryRequest(ctx, async () => {
    const users = await getUserList(ctx.request.query.owner as string | undefined);
    const nodes = await getNodeList(ctx.request.query.node as string | undefined);
...
async function getJobSettings(users: string[], nodes: NodeObj[]) {
  ...
  for (const node of nodes) {
    const dirs = (await listUserFolders(node, CONFIG_FILE_NAME)).directories.filter((_: any) =>
      users.includes(_.owner)
    );
    for (const dir of dirs) {
      ...
      try {
        ...
        setting.error = (err as Error).message; // TODO
        settings.push(setting);
        continue;
      }
